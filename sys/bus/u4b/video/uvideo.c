...
#define DEVNAME(_s) (device_get_name(_s->sc_dev))
...
#define	UVIDEO_MAX_VS_NUM			 8
	...
	usb_error_t				(*sc_decode_stream_header)
						    (struct uvideo_softc *,
						    uint8_t *, int);
};
...
usb_error_t	uvideo_vc_parse_desc(struct uvideo_softc *);
usb_error_t	uvideo_vc_parse_desc_header(struct uvideo_softc *,
		    const usb_descriptor_t *);
usb_error_t	uvideo_vc_parse_desc_pu(struct uvideo_softc *,
		    const usb_descriptor_t *);
usb_error_t	uvideo_vc_get_ctrl(struct uvideo_softc *, uint8_t *, uint8_t,
...
usb_error_t	uvideo_vs_parse_desc_input_header(struct uvideo_softc *,
		    const usb_descriptor_t *);
usb_error_t	uvideo_vs_parse_desc_format(struct uvideo_softc *);
usb_error_t	uvideo_vs_parse_desc_format_mjpeg(struct uvideo_softc *,
		    const usb_descriptor_t *);
usb_error_t	uvideo_vs_parse_desc_format_uncompressed(struct uvideo_softc *,
		    const usb_descriptor_t *);
usb_error_t	uvideo_vs_parse_desc_frame(struct uvideo_softc *);
usb_error_t	uvideo_vs_parse_desc_frame_sub(struct uvideo_softc *,
		    const usb_descriptor_t *);
...
void		uvideo_find_res(struct uvideo_softc *, int, int, int,
		    struct uvideo_res *);
usb_error_t	uvideo_vs_negotiation(struct uvideo_softc *, int);
usb_error_t	uvideo_vs_set_probe(struct uvideo_softc *, uint8_t *);
usb_error_t	uvideo_vs_get_probe(struct uvideo_softc *, uint8_t *, uint8_t);
usb_error_t	uvideo_vs_set_commit(struct uvideo_softc *, uint8_t *);
usb_error_t	uvideo_vs_alloc_frame(struct uvideo_softc *);
void		uvideo_vs_free_frame(struct uvideo_softc *);
usb_error_t	uvideo_vs_alloc_isoc(struct uvideo_softc *);
usb_error_t	uvideo_vs_alloc_bulk(struct uvideo_softc *);
void		uvideo_vs_free_isoc(struct uvideo_softc *);
void		uvideo_vs_free_bulk(struct uvideo_softc *);
usb_error_t	uvideo_vs_open(struct uvideo_softc *);
void		uvideo_vs_close(struct uvideo_softc *);
usb_error_t	uvideo_vs_init(struct uvideo_softc *);
...
void		uvideo_vs_cb(struct usbd_xfer *, void *,
		    usb_error_t);
usb_error_t	uvideo_vs_decode_stream_header(struct uvideo_softc *,
		    uint8_t *, int); 
usb_error_t	uvideo_vs_decode_stream_header_isight(struct uvideo_softc *,
		    uint8_t *, int);
int		uvideo_mmap_queue(struct uvideo_softc *, uint8_t *, int);
void		uvideo_read(struct uvideo_softc *, uint8_t *, int);
usb_error_t	uvideo_usb_control(struct uvideo_softc *, uint8_t, uint8_t,
...
uvideo_attach(struct device *parent, struct device *self, void *aux)
{
	...
	if (sc->sc_quirk && sc->sc_quirk->flags & UVIDEO_FLAG_NOATTACH) {
		kprintf("%s: device not supported\n", DEVNAME(sc));
		return;
	}
...
uvideo_attach_hook(struct device *self)
{
	...
	DPRINTF("uvideo_attach: doing video_attach_mi\n");
...
uvideo_find_ctrl(struct uvideo_softc *sc, int id)
{
	...
	if (sc->sc_desc_vc_pu_num == 0) {
		...
		DPRINTF("%s: %s: no processing unit descriptors found!\n",
		    DEVNAME(sc), __func__);
		return (EINVAL);
	}
	...
	if (found == 0) {
		DPRINTF("%s: %s: control not supported by driver!\n",
		    DEVNAME(sc), __func__);
		return (EINVAL);
	}
	...
	if (found == 0) {
		DPRINTF("%s: %s: control not supported by device!\n",
		    DEVNAME(sc), __func__);
		return (EINVAL);
	}
...
uvideo_vs_parse_desc(struct uvideo_softc *sc, usb_config_descriptor_t *cdesc)
{
	...
	DPRINTF("%s: number of total interfaces=%d\n",
    ...
    const usb_descriptor_t *desc)
{
	...
	if (desc->bDescriptorSubtype == UDESCSUB_VS_FRAME_UNCOMPRESSED) {
		...
		DPRINTFN(10, "%s: %s: frame buffer size=%d "
...
uvideo_vs_parse_desc_alt(struct uvideo_softc *sc, int vs_nr, int iface, int numalts)
{
	...
	while (desc) {
		...
		if (id->bInterfaceNumber != iface)
			goto next;
		DPRINTF("%s: bAlternateSetting=0x%02x, ",
		    DEVNAME(sc), id->bAlternateSetting);
		if (id->bNumEndpoints == 0) {
			DPRINTF("no endpoint descriptor\n");
			goto next;
		}
		...
		DPRINTF("bEndpointAddress=0x%02x, ", ed->bEndpointAddress);
		DPRINTF("wMaxPacketSize=%d\n", UGETW(ed->wMaxPacketSize));
...
next:
	...
	DPRINTF("bAlternateSetting=0x%02x psize=%d max_packet_size=%d\n",
    ...
    struct uvideo_res *r)
{
	...
	for (i = 0; i < sc->sc_fmtgrp[idx].frame_num; i++) {
		...
		DPRINTF("%s: %s: frame index %d: width=%d, height=%d\n",
...
uvideo_vs_negotiation(struct uvideo_softc *sc, int commit)
{
	...
	if (fmtgrp->frame_num == 0) {
		kprintf("%s: %s: no frame descriptors found!\n",
		    __func__, DEVNAME(sc));
		return (USB_ERR_INVAL);
	}
	...
	if (sc->sc_frame_rate != 0) {
		...
		if (!nivals && (ival_bytes >= sizeof(uDWord) * 3)) {
			...
			DPRINTF("%s: %s: bad frame ival descriptor\n",
			    DEVNAME(sc), __func__);
		}
	}
	...
	if (pc->bFormatIndex != fmtgrp->format->bFormatIndex) {
		changed++;
		DPRINTF("%s: %s: wanted format 0x%x, got format 0x%x\n",
		...
		if (i == sc->sc_fmtgrp_num) {
			DPRINTF("%s: %s: invalid format index 0x%x\n",
	...
	if (pc->bFrameIndex != fmtgrp->frame_cur->bFrameIndex) {
		changed++;
		DPRINTF("%s: %s: wanted frame 0x%x, got frame 0x%x\n",
		...
		if (i == fmtgrp->frame_num) {
			DPRINTF("%s: %s: invalid frame index 0x%x\n",
	...
	if (frame->bDescriptorSubtype == UDESCSUB_VS_FRAME_UNCOMPRESSED) {
		...
		DPRINTF("fixed dwMaxVideoFrameSize=%d, "
		    "width=%d height=%d bpp=%d\n",
		...
		if (UGETDW(pc->dwMaxVideoFrameSize) == 0 &&
			...
			DPRINTF("%s: dwMaxVideoFrameSize == 0, fixed\n",
			    DEVNAME(sc));
			USETDW(pc->dwMaxVideoFrameSize, 
...
uvideo_vs_set_probe(struct uvideo_softc *sc, uint8_t *probe_data)
{
	...
	DPRINTF("%s: SET probe request successfully\n", DEVNAME(sc));

	DPRINTF("bmHint=0x%02x\n", UGETW(pc->bmHint));
	DPRINTF("bFormatIndex=0x%02x\n", pc->bFormatIndex);
	DPRINTF("bFrameIndex=0x%02x\n", pc->bFrameIndex);
	DPRINTF("dwFrameInterval=%d (100ns units)\n",
	    UGETDW(pc->dwFrameInterval));
	DPRINTF("wKeyFrameRate=%d\n", UGETW(pc->wKeyFrameRate));
	DPRINTF("wPFrameRate=%d\n", UGETW(pc->wPFrameRate));
	DPRINTF("wCompQuality=%d\n", UGETW(pc->wCompQuality));
	DPRINTF("wCompWindowSize=%d\n", UGETW(pc->wCompWindowSize));
	DPRINTF("wDelay=%d (ms)\n", UGETW(pc->wDelay));
	DPRINTF("dwMaxVideoFrameSize=%d (bytes)\n",
	    UGETDW(pc->dwMaxVideoFrameSize));
	DPRINTF("dwMaxPayloadTransferSize=%d (bytes)\n",
    ...
    uint8_t request)
{
	...
	DPRINTF("%s: GET probe request successfully\n", DEVNAME(sc));
...
uvideo_vs_set_commit(struct uvideo_softc *sc, uint8_t *probe_data)
{
	...
	DPRINTF("%s: SET commit request successfully\n", DEVNAME(sc));
...
uvideo_vs_alloc_frame(struct uvideo_softc *sc)
{
	...
	if (sc->sc_max_fbuf_size < fb->buf_size &&
	    sc->sc_mmap_flag == 0) {
		kprintf("%s: software video buffer is too small!\n",
		    DEVNAME(sc));
		return (USBD_NOMEM);
	}
	...
	DPRINTF("%s: %s: allocated %d bytes frame buffer\n",
...
uvideo_vs_alloc_isoc(struct uvideo_softc *sc)
{
	...
	for (i = 0; i < UVIDEO_IXFERS; i++) {
		...
		if (sc->sc_vs_cur->ixfer[i].xfer == NULL) {
			kprintf("%s: could not allocate isoc VS xfer!\n",
			    DEVNAME(sc));
			return (USBD_NOMEM);	
		}
		...
		if (sc->sc_vs_cur->ixfer[i].buf == NULL) {
			kprintf("%s: could not allocate isoc VS buffer!\n",
			    DEVNAME(sc));
			return (USBD_NOMEM);
		}
		DPRINTF("%s: allocated %d bytes isoc VS xfer buffer\n",
		    DEVNAME(sc), size);
	}
...
uvideo_vs_alloc_bulk(struct uvideo_softc *sc)
{
	...
	if (sc->sc_vs_cur->bxfer.xfer == NULL) {
		kprintf("%s: could not allocate bulk VS xfer!\n",
		    DEVNAME(sc));
		return (USBD_NOMEM);
	}
	...
	if (sc->sc_vs_cur->bxfer.buf == NULL) {
		kprintf("%s: could not allocate bulk VS buffer!\n",
		    DEVNAME(sc));
		return (USBD_NOMEM);
	}
	DPRINTF("%s: allocated %d bytes bulk VS xfer buffer\n",
	    DEVNAME(sc), size);
...
uvideo_vs_open(struct uvideo_softc *sc)
{
	...
	if (error != USB_ERR_NORMAL_COMPLETION) {
		kprintf("%s: could not set alternate interface!\n",
		    DEVNAME(sc));
		return (error);
	}
	...
	if (ed == NULL) {
		kprintf("%s: no endpoint descriptor for VS iface\n",
		    DEVNAME(sc));
		return (USB_ERR_INVAL);
	}

	DPRINTF("%s: open pipe for bEndpointAddress=0x%02x\n",
	...
	error = usbd_open_pipe(
	    ...
	    &sc->sc_vs_cur->pipeh);
	if (error != USB_ERR_NORMAL_COMPLETION) {
		kprintf("%s: could not open VS pipe: %s\n",
		    DEVNAME(sc), usbd_errstr(error));
		return (error);
	}
...
uvideo_vs_start_bulk(struct uvideo_softc *sc)
{
	...
	error = kthread_create(uvideo_vs_start_bulk_thread, sc, NULL,
	    DEVNAME(sc));
	if (error) {
		kprintf("%s: can't create kernel thread!", DEVNAME(sc));
		return (error);
	}
...
uvideo_vs_start_bulk_thread(void *arg)
{
	...
	while (sc->sc_vs_cur->bulk_running) {
		...
		if (error != USB_ERR_NORMAL_COMPLETION) {
			DPRINTF("%s: error in bulk xfer: %s!\n",
			    DEVNAME(sc), usbd_errstr(error));
			break;
		}

		DPRINTFN(2, "%s: *** buffer len = %d\n", DEVNAME(sc), size);
    ...
    struct uvideo_isoc_xfer *ixfer)
{
	...
	if (error && error != USBD_IN_PROGRESS) {
		DPRINTF("%s: usbd_transfer error=%s!\n",
		    DEVNAME(sc), usbd_errstr(error));
	}
}
    ...
    int frame_size)
{
	...
	DPRINTFN(2, "%s: stream header len = %d\n", DEVNAME(sc), sh->bLength);
	...
	if (sh->bFlags & UVIDEO_SH_FLAG_ERR) {
		...
		DPRINTF("%s: %s: stream error!\n", DEVNAME(sc), __func__);
		return (USBD_CANCELLED);
	}

	DPRINTFN(2, "%s: frame_size = %d\n", DEVNAME(sc), frame_size);

	if (sh->bFlags & UVIDEO_SH_FLAG_FID) {
		DPRINTFN(2, "%s: %s: FID ON (0x%02x)\n",
		    DEVNAME(sc), __func__,
		    sh->bFlags & UVIDEO_SH_FLAG_FID);
	} else {
		DPRINTFN(2, "%s: %s: FID OFF (0x%02x)\n",
		    DEVNAME(sc), __func__,
		    sh->bFlags & UVIDEO_SH_FLAG_FID);
	}
	...
	if (fb->sample == 0) {
		...
		if (fb->fid != (sh->bFlags & UVIDEO_SH_FLAG_FID)) {
			DPRINTF("%s: %s: wrong FID, ignore last frame!\n",
			    DEVNAME(sc), __func__);
			fb->sample = 1;
			fb->fid = sh->bFlags & UVIDEO_SH_FLAG_FID;
			fb->offset = 0;
...
uvideo_mmap_queue(struct uvideo_softc *sc, uint8_t *buf, int len)
{
	...
	if (i == sc->sc_mmap_count) {
		DPRINTF("%s: %s: mmap queue is full!",
		    DEVNAME(sc), __func__);
		return ENOMEM;
	}
	...
	DPRINTFN(2, "%s: %s: frame queued on index %d\n",
...
uvideo_dump_desc_all(struct uvideo_softc *sc)
{
	...
	while (desc) {
		kprintf("bLength=%d\n", desc->bLength);
		kprintf("bDescriptorType=0x%02x", desc->bDescriptorType);

		switch (desc->bDescriptorType) {
		case UDESC_CS_INTERFACE:
			kprintf(" (CS_INTERFACE)\n");

			switch (desc->bDescriptorSubtype) {
			case UDESCSUB_VC_HEADER:
				kprintf("bDescriptorSubtype=0x%02x",
				    desc->bDescriptorSubtype);
				if (uvideo_desc_len(desc, 12, 11, 1, 0)) {
					kprintf(" (UDESCSUB_VC_HEADER)\n");
					kprintf("|\n");
					uvideo_dump_desc_vc_header(sc, desc);
					break;
				}
				if (uvideo_desc_len(desc, 13, 3, 0, 12)) {
					kprintf(" (UDESCSUB_VS_INPUT_HEADER)\n");
					kprintf("|\n");
			...
			case UDESCSUB_VC_INPUT_TERMINAL:
				kprintf("bDescriptorSubtype=0x%02x",
				    desc->bDescriptorSubtype);
				kprintf(" (UDESCSUB_VC_INPUT_TERMINAL)\n");
				kprintf("|\n");
			...
			case UDESCSUB_VC_SELECTOR_UNIT:
				kprintf("bDescriptorSubtype=0x%02x",
				    desc->bDescriptorSubtype);
				if (desc->bLength == 27) {
					kprintf(" (UDESCSUB_VS_FORMAT_"
					    "UNCOMPRESSED)\n");
					uvideo_dump_desc_format_uncompressed(
					    sc, desc);
				} else {
					kprintf(" (UDESCSUB_VC_SELECTOR_"
					    "UNIT)\n");
					/* TODO */
				}
				break;
			case UDESCSUB_VC_PROCESSING_UNIT:
				...
				if (desc->bLength >
				    sizeof(struct usb_video_frame_desc)) {
					kprintf(" (UDESCSUB_VS_FRAME_"
					    "UNCOMPRESSED)\n");
					uvideo_dump_desc_frame(sc, desc);
				} else {
					kprintf(" (UDESCSUB_VC_PROCESSING_"
					    "UNIT)\n");
					kprintf("|\n");
			...
			case UDESCSUB_VC_EXTENSION_UNIT:
				kprintf("bDescriptorSubtype=0x%02x",
				    desc->bDescriptorSubtype);
				if (desc->bLength == 11) {
					kprintf(" (UDESCSUB_VS_FORMAT_MJPEG)\n");
					kprintf("|\n");
					uvideo_dump_desc_format_mjpeg(sc, desc);
				} else {
					kprintf(" (UDESCSUB_VC_EXTENSION_"
					    "UNIT)\n");
					kprintf("|\n");
			...
			case UDESCSUB_VS_FRAME_MJPEG:
				kprintf("bDescriptorSubtype=0x%02x",
				    desc->bDescriptorSubtype);
				kprintf(" (UDESCSUB_VS_FRAME_MJPEG)\n");
				if (desc->bLength >
				    sizeof(struct usb_video_frame_desc)) {
					kprintf("|\n");
					uvideo_dump_desc_frame(sc, desc);
			...
			case UDESCSUB_VS_COLORFORMAT:
				kprintf("bDescriptorSubtype=0x%02x",
				   desc->bDescriptorSubtype);
				kprintf(" (UDESCSUB_VS_COLORFORMAT)\n");
				kprintf("|\n");
		...
		case UDESC_CS_ENDPOINT:
			kprintf(" (UDESC_CS_ENDPOINT)\n");

			switch (desc->bDescriptorSubtype) {
			case EP_INTERRUPT:
				kprintf("bDescriptorSubtype=0x%02x",
				    desc->bDescriptorSubtype);
				kprintf(" (EP_INTERRUPT)\n");
				kprintf("|\n");
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bcdUVC=0x%04x\n", UGETW(d->bcdUVC));
	kprintf("wTotalLength=%d\n", UGETW(d->wTotalLength));
	kprintf("dwClockFrequency=%d\n", UGETDW(d->dwClockFrequency));
	kprintf("bInCollection=0x%02x\n", d->bInCollection);
}
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bDescriptorType=0x%02x\n", d->bDescriptorType);
	kprintf("bDescriptorSubtype=0x%02x\n", d->bDescriptorSubtype);
	kprintf("bNumFormats=%d\n", d->bNumFormats);
	kprintf("wTotalLength=%d\n", UGETW(d->wTotalLength));
	kprintf("bEndpointAddress=0x%02x\n", d->bEndpointAddress);
	kprintf("bmInfo=0x%02x\n", d->bmInfo);
	kprintf("bTerminalLink=0x%02x\n", d->bTerminalLink);
	kprintf("bStillCaptureMethod=0x%02x\n", d->bStillCaptureMethod);
	kprintf("bTriggerSupport=0x%02x\n", d->bTriggerSupport);
	kprintf("bTriggerUsage=0x%02x\n", d->bTriggerUsage);
	kprintf("bControlSize=%d\n", d->bControlSize);
}
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bDescriptorType=0x%02x\n", d->bDescriptorType);
	kprintf("bDescriptorSubtype=0x%02x\n", d->bDescriptorSubtype);
	kprintf("bTerminalID=0x%02x\n", d->bTerminalID);
	kprintf("wTerminalType=0x%04x\n", UGETW(d->wTerminalType));
	kprintf("bAssocTerminal=0x%02x\n", d->bAssocTerminal);
	kprintf("iTerminal=0x%02x\n", d->iTerminal);
}
    ...
    const usb_descriptor_t *desc)
{
	...
	if (UE_GET_XFERTYPE(d->bmAttributes) == UE_INTERRUPT)
		kprintf(" (UE_INTERRUPT)\n");
	kprintf("wMaxPacketSize=%d\n", UGETW(d->wMaxPacketSize));
	kprintf("bInterval=0x%02x\n", d->bInterval);
}
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bLength=%d\n", d->bLength);
	kprintf("bDescriptorType=0x%02x\n", d->bDescriptorType);
	kprintf("bFirstInterface=0x%02x\n", d->bFirstInterface);
	kprintf("bInterfaceCount=%d\n", d->bInterfaceCount);
	kprintf("bFunctionClass=0x%02x\n", d->bFunctionClass);
	kprintf("bFunctionSubClass=0x%02x\n", d->bFunctionSubClass);
	kprintf("bFunctionProtocol=0x%02x\n", d->bFunctionProtocol);
	kprintf("iFunction=0x%02x\n", d->iFunction);
}
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bLength=%d\n", d->bLength);
	kprintf("bDescriptorType=0x%02x\n", d->bDescriptorType);
	kprintf("bInterfaceNumber=0x%02x\n", d->bInterfaceNumber);
	kprintf("bAlternateSetting=0x%02x\n", d->bAlternateSetting);
	kprintf("bNumEndpoints=%d\n", d->bNumEndpoints);
	kprintf("bInterfaceClass=0x%02x\n", d->bInterfaceClass);
	kprintf("bInterfaceSubClass=0x%02x\n", d->bInterfaceSubClass);
	kprintf("bInterfaceProtocol=0x%02x\n", d->bInterfaceProtocol);
	kprintf("iInterface=0x%02x\n", d->iInterface);
}
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bDescriptorType=0x%02x\n", d->bDescriptorType);
	kprintf("bDescriptorSubtype=0x%02x\n", d->bDescriptorSubtype);
	kprintf("bColorPrimaries=0x%02x\n", d->bColorPrimaries);
	kprintf("bTransferCharacteristics=0x%02x\n",
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bDescriptorType=0x%02x\n", d->bDescriptorType);
	kprintf("bDescriptorSubtype=0x%02x\n", d->bDescriptorSubtype);
	kprintf("bFormatIndex=0x%02x\n", d->bFormatIndex);
	kprintf("bNumFrameDescriptors=0x%02x\n", d->bNumFrameDescriptors);
	kprintf("bmFlags=0x%02x\n", d->bmFlags);
	kprintf("bDefaultFrameIndex=0x%02x\n", d->bDefaultFrameIndex);
	kprintf("bAspectRatioX=0x%02x\n", d->bAspectRatioX);
	kprintf("bAspectRatioY=0x%02x\n", d->bAspectRatioY);
	kprintf("bmInterlaceFlags=0x%02x\n", d->bmInterlaceFlags);
	kprintf("bCopyProtect=0x%02x\n", d->bCopyProtect);
}
...
uvideo_dump_desc_frame(struct uvideo_softc *sc, const usb_descriptor_t *desc)
{
	...
	kprintf("bDescriptorType=0x%02x\n", d->bDescriptorType);
	kprintf("bDescriptorSubtype=0x%02x\n", d->bDescriptorSubtype);
	kprintf("bFrameIndex=0x%02x\n", d->bFrameIndex);
	kprintf("bmCapabilities=0x%02x\n", d->bmCapabilities);
	kprintf("wWidth=%d\n", UGETW(d->wWidth));
	kprintf("wHeight=%d\n", UGETW(d->wHeight));
	kprintf("dwMinBitRate=%d\n", UGETDW(d->dwMinBitRate));
	kprintf("dwMaxBitRate=%d\n", UGETDW(d->dwMaxBitRate));
	kprintf("dwMaxVideoFrameBufferSize=%d\n",
	    UGETDW(d->dwMaxVideoFrameBufferSize));
	kprintf("dwDefaultFrameInterval=%d\n",
	...
	if (!d->bFrameIntervalType) {
		...
		if (d->bLength < (sizeof(struct usb_video_frame_desc) +
		    sizeof(uDWord) * 3)) {
			kprintf("invalid frame descriptor length\n");
		} else {
			kprintf("dwMinFrameInterval = %d\n", UGETDW(p));
			p += sizeof(uDWord);
			kprintf("dwMaxFrameInterval = %d\n", UGETDW(p));
			p += sizeof(uDWord);
			kprintf("dwFrameIntervalStep = %d\n", UGETDW(p));
			p += sizeof(uDWord);
		}
	} else {
		/* discrete */
		...
		for (i = 0; i < d->bFrameIntervalType; i++) {
			if (length <= 0) {
				kprintf("frame descriptor ended early\n");
				break;
			}
			kprintf("dwFrameInterval = %d\n", UGETDW(p));
			p += sizeof(uDWord);
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bFormatIndex=0x%02x\n", d->bFormatIndex);
	kprintf("bNumFrameDescriptors=0x%02x\n", d->bNumFrameDescriptors);
	kprintf("guidFormat=%s\n", d->guidFormat);
	kprintf("bBitsPerPixel=0x%02x\n", d->bBitsPerPixel);
	kprintf("bDefaultFrameIndex=0x%02x\n", d->bDefaultFrameIndex);
	kprintf("bAspectRatioX=0x%02x\n", d->bAspectRatioX);
	kprintf("bAspectRatioY=0x%02x\n", d->bAspectRatioY);
	kprintf("bmInterlaceFlags=0x%02x\n", d->bmInterlaceFlags);
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bDescriptorType=0x%02x\n", d->bDescriptorType);
	kprintf("bDescriptorSubtype=0x%02x\n", d->bDescriptorSubtype);
	kprintf("bUnitID=0x%02x\n", d->bUnitID);
	kprintf("bSourceID=0x%02x\n", d->bSourceID);
	kprintf("wMaxMultiplier=%d\n", UGETW(d->wMaxMultiplier));
	kprintf("bControlSize=%d\n", d->bControlSize);
	kprintf("bmControls=0x");
	uvideo_hexdump(d->bmControls, d->bControlSize, 1);
	kprintf("iProcessing=0x%02x\n", d->bmControls[d->bControlSize]);
	kprintf("bmVideoStandards=0x%02x\n", d->bmControls[d->bControlSize + 1]);
}
    ...
    const usb_descriptor_t *desc)
{
	...
	kprintf("bDescriptorSubtype=0x%02x\n", d->bDescriptorSubtype);
	kprintf("bUnitID=0x%02x\n", d->bUnitID);
	kprintf("guidExtensionCode=0x");
	uvideo_hexdump(d->guidExtensionCode, sizeof(d->guidExtensionCode), 1);
	kprintf("bNumControls=0x%02x\n", d->bNumControls);
	kprintf("bNrInPins=0x%02x\n", d->bNrInPins);
}
...
uvideo_debug_file_open(struct uvideo_softc *sc)
{
	...
	if (error) {
		DPRINTF("%s: %s: can't create debug file %s!\n",
...
uvideo_s_fmt(void *v, struct v4l2_format *fmt)
{
	...
	DPRINTF("%s: %s: requested width=%d, height=%d\n",
	...
	DPRINTF("%s: %s: offered width=%d, height=%d\n",
...
uvideo_g_fmt(void *v, struct v4l2_format *fmt)
{
	...
	DPRINTF("%s: %s: current width=%d, height=%d\n",
	    DEVNAME(sc), __func__, fmt->fmt.pix.width, fmt->fmt.pix.height);
...
uvideo_reqbufs(void *v, struct v4l2_requestbuffers *rb)
{
	...
	if (sc->sc_mmap_count > 0 || sc->sc_mmap_buffer != NULL) {
		DPRINTF("%s: %s: mmap buffers already allocated\n",
		    DEVNAME(sc), __func__);
		return (EINVAL);
	}
	...
	if (buf_size >= SIZE_MAX / UVIDEO_MAX_BUFFERS) {
		kprintf("%s: video frame size too large!\n", DEVNAME(sc));
		sc->sc_mmap_count = 0;
		return (EINVAL);
	}
	...
	DPRINTF("%s: allocated %d bytes mmap buffer\n",
	...
	for (i = 0; i < sc->sc_mmap_count; i++) {
		...
		DPRINTF("%s: %s: index=%d, offset=%d, length=%d\n",
		    DEVNAME(sc), __func__,
...
uvideo_qbuf(void *v, struct v4l2_buffer *qb)
{
	...
	DPRINTFN(2, "%s: %s: buffer on index %d ready for queueing\n",
...
uvideo_dqbuf(void *v, struct v4l2_buffer *dqb)
{
	...
	DPRINTFN(2, "%s: %s: frame dequeued from index %d\n",
...
uvideo_queryctrl(void *v, struct v4l2_queryctrl *qctrl)
{
	...
	if (ctrl_len < 1 || ctrl_len > 2) {
		kprintf("%s: invalid control length: %d\n", __func__, ctrl_len);
		return (EINVAL);
	}
...
uvideo_ucode_loader_ricoh(struct uvideo_softc *sc)
{
	...
	if (error != USB_ERR_NORMAL_COMPLETION) {
		kprintf("%s: ucode status error=%s!\n",
		    DEVNAME(sc), usbd_errstr(error));
		return (USB_ERR_INVAL);
	}
	if (cbuf) {
		DPRINTF("%s: microcode already loaded\n", DEVNAME(sc));
		return (USB_ERR_NORMAL_COMPLETION);
	} else {
		DPRINTF("%s: microcode not loaded\n", DEVNAME(sc));
	}

	/* open microcode file */
	error = loadfirmware(sc->sc_quirk->ucode_name, &ucode, &ucode_size);
	if (error != 0) {
		kprintf("%s: loadfirmware error=%d!\n", DEVNAME(sc), error);
		return (USB_ERR_INVAL);
	}
	...
	while (remain > 0) {
		...
		DPRINTF("%s: uploaded %d bytes ucode to addr 0x%x\n",
		    DEVNAME(sc), len, addr);
...
uvideo_ucode_loader_apple_isight(struct uvideo_softc *sc)
{
	...
	error = uvideo_usb_control(sc, UT_WRITE_VENDOR_DEVICE, 0xa0, 0xe600,
	    &cbuf, sizeof(cbuf));
	if (error) {
		kprintf("%s: failed to init firmware loading state: %s\n",
		    DEVNAME(sc), usbd_errstr(error));
		return (error);
	}
	...
	while (code < ucode + ucode_size) {
		...
		len = (code[0] << 8) | code[1];
		req = (code[2] << 8) | code[3];
		DPRINTF("%s: ucode data len=%d, request=0x%x\n",
		    DEVNAME(sc), len, req);
		if (len < 1 || len > 1023) {
		...
		for (off = 0; len > 0; req += 50, off += 50) {
			...
			DPRINTF("%s: send %d bytes data to offset 0x%x\n",
			    DEVNAME(sc), llen, req);
			error = uvideo_usb_control(sc, UT_WRITE_VENDOR_DEVICE,
			    0xa0, req, code, llen);
			if (error) {
