...
#include <dev/netif/wpi/if_wpi_debug.h>
    ...
    const uint8_t mac[IEEE80211_ADDR_LEN])
{
	...
	wvp = kmalloc(sizeof(struct wpi_vap), M_80211_VAP, M_WAITOK | M_ZERO);
	vap = &wvp->wv_vap;
...
wpi_node_alloc(struct ieee80211vap *vap, const uint8_t mac[IEEE80211_ADDR_LEN])
{
	...
	wn = kmalloc(sizeof (struct wpi_node), M_80211_NODE,
	    M_INTWAIT | M_ZERO);
...
wpi_fatal_intr(struct wpi_softc *sc)
{
	...
	kprintf("firmware error log (count = %u):\n", count);
	for (i = 0; i < count; i++) {
...
wpi_scan(struct wpi_softc *sc, struct ieee80211_channel *c)
{
	...
	buf = kmalloc(WPI_SCAN_MAXSZ, M_DEVBUF, M_INTWAIT | M_ZERO);
	if (buf == NULL) {
		device_printf(sc->sc_dev,
